...
### 2.2 在 Claude Code 中怎么用——一个 Markdown 文件，放到指定目录：
...
### 3.3 在 Claude Code 中怎么用交互式配置**，或直接编辑 `settings.json`。
...
### 3.6 优劣速览、不依赖 AI 记忆、支持阻止危险操作、四种类型覆盖多种场景 |
...
### 4.5 优劣速览、工具权限隔离、模型可选（控制成本）、支持并行和持久记忆    |
...
### 8.1 Have you prepared for AlphaGo?

2016 年，AlphaGo 击败李世乭。围棋界没有消亡，但围棋的训练方式、竞技格局、甚至对弈的意义，都被永久改变了。

今天的 Agent，仿佛也正处在类似的拐点。你准备好了吗？

![](alphago.webp)

### 8.2 Enthusiasm, or just a job?
